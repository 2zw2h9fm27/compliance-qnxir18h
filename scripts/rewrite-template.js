...
function extractRelatedTerms(title, type) {
  const terms = [];
  const lowerTitle = title.toLowerCase();
  
  if (type === 'rhetorician') {
    if (lowerTitle.includes('aristotle')) terms.push('Plato', 'Cicero', 'Rhetoric', 'Stasis Theory');
    else if (lowerTitle.includes('cicero')) terms.push('Aristotle', 'Quintilian', 'Rhetoric', 'Inventio');
    else if (lowerTitle.includes('quintilian')) terms.push('Cicero', 'Aristotle', 'Progymnasmata', 'Rhetoric');
    ...
    terms.push('Epistrophe', 'Symploce', 'Antanaclasis', 'Anadiplosis');
  } else if (lowerTitle.includes('antimetabole')) {
    terms.push('Anaphora', 'Antanaclasis', 'Chiasmus', 'Symploce');
    ...
    terms.push('Logos', 'Pathos', 'Kairos', 'Inventio');
  } else if (lowerTitle.includes('logos')) {
    terms.push('Ethos', 'Pathos', 'Kairos', 'Enthymeme');
...
function buildTemplate(title, description, type, body, bibliography, filename) {
  const cleanBody = cleanBodyText(body);
  
  const paragraphs = cleanBody.split('\n\n').filter(p => p.trim().length > 0);
  const introParagraphs = paragraphs.slice(0, 2).join('\n\n');
  
  const dataGrid = generateDataGrid(title, type, cleanBody, filename);
  ...
  const relatedTerms = extractRelatedTerms(title, type);
...
function processFile(file) {
  ...
  frontmatter = fixFrontmatterDescription(frontmatter, body);
  ...
  const template = buildTemplate(title, description, type, cleanBody, bibliography, file);
